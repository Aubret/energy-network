...
*
*   DATE:	    June 2002
*   REVISION:   22 June 2003, 19 January 2005, 1 May 2005
*
*   DOCUMENTATION:
...
/*********************************************************/
    	...
    	public void setSubMatrix(int i, int j, int k, int l, Complex[][] smat){
        	...
        	for(int p=0; p<n; p++){
            		for(int q=0; q<m; p++){
                		this.matrix[i+p][j+q] = Complex.copy(smat[i][j]);
            		}
        	}
    	}
    	...
    	public void setSubMatrix(int[] row, int[] col, Complex[][] smat){
        	int n=row.length;
        	int m=col.length;
        	for(int p=0; p<n; p++){
            		for(int q=0; q<m; p++){
                		this.matrix[row[p]][col[q]] = Complex.copy(smat[p][q]);
            		}
        	}
    	}
    	...
    	public ComplexMatrix getSubMatrix(int i, int j, int k, int l){
        	...
        	if(j>l)throw new IllegalArgumentException("column indices inverted");
        	int n=k-i+1, m=j-l+1;
        	ComplexMatrix smat = new ComplexMatrix(n, m);
        	Complex[][] sarray = this.getArrayReference();
        	for(int p=0; p<n; p++){
            		for(int q=0; q<m; p++){
